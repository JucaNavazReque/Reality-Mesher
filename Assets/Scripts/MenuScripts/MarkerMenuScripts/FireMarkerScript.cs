...
public class FireMarkerScript : MonoBehaviour
{
    ...
    public void createMarker() {
        ...
        if (Physics.Raycast(ray, out hitData, Mathf.Infinity, _layerMask))
        {
            GameObject realMarker = Instantiate(markerPrefab, hitData.point, rotation_quat);
            realMarker.tag = "Marker";
            realMarker.name = MarkerName;
            realMarker.transform.GetChild(0).GetComponent<Renderer>().material.color = MarkerColor;
            if (HighlightMarkers) {realMarker.transform.Find("Sphere").GetComponent<Renderer>().material.shader = HighlightShader;}
            GameObject DeleteMarkerButton = Instantiate(deleteButton, realMarker.transform.position, rotation_quat);
            DeleteMarkerButton.transform.Translate(0f, 0.048f, 0f);
            DeleteMarkerButton.transform.SetParent(realMarker.transform);

            // Add the marker to the MarkerManager
            Debug.Log(MarkerManager.Instance);
            Debug.Log(realMarker);
            MarkerManager.Instance.AddMarker(realMarker);
        }
    }
